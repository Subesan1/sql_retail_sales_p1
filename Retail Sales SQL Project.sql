...
select count(distinct customer_id) as Total_coustomers from retail_sales;

-- How many categoires we have?
...
-- rank() over(partition by year(sale_date)) order by avg(total_sale) desc) as ramk 
